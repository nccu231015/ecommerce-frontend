...
### 🔄 AI 混合搜索 (核心特色)
- **混合搜索引擎**: 結合向量語義搜索和全文搜索，使用 MongoDB $rankFusion
- **動態權重調整**: 根據查詢類型自動優化搜索策略
  - 品牌查詢: 全文搜索權重 70%
  - 描述性查詢: 向量搜索權重 80%
  - 預設平衡: 向量 60% + 全文 40%
- **智能降級機制**: 向量搜索失敗時自動降級到全文搜索
...
3. **MongoDB $rankFusion** 同時執行：
   - **向量搜索**: 理解語義相似性
   - **全文搜索**: 精確關鍵詞匹配
4. **RRF 算法** 合併兩種搜索結果，計算綜合排名
